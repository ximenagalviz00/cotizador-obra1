...
# ============================================================
# COTIZADOR + PROYECCIÓN — versión web (Streamlit)
# Adaptación completa de notebook original (2025)
# Diseño negro/naranja (#F87B1B)
# ============================================================
...
# ---------------------------------------
st.set_page_config(page_title="Cotizador de Remodelación", layout="wide")
...
CATALOGO_NEGRA = {
    "Partidas de Obra Negra": {
        "Demolición controlada (m²)": 50000,
        "Cimentación corridas (m³)": 320000,
        "Estructura de concreto (m³)": 450000,
        "Mampostería/muros (m²)": 120000,
    }
}
...
LEVEL_MULT   = {"Básico": 0.90, "Estándar": 1.00, "Premium": 1.40}
INTEL_PLUS   = 0.15
IVA = 0.19
...
# UI PRINCIPAL
# ---------------------------------------
st.title("🏗️ Cotizador de Remodelación")
...
st.divider()
st.subheader("1️⃣ Agregar ítems a la cotización")

if 'items' not in st.session_state:
    st.session_state.items = []

col1, col2 = st.columns(2)
categoria = col1.selectbox("Categoría", list(CATALOGO.keys()))
material = col2.selectbox("Ítem/partida", list(CATALOGO[categoria].keys()))
...
if obra_tipo.startswith("Obra blanca"):
    ...
    inteligente = st.checkbox("Opción Inteligente (+15%)")
...
st.write(f"💰 **Subtotal estimado:** {form_cop(subtotal)}")
...
if st.session_state.items:
    ...
    ax.set_title("Distribución de costos por categoría")
    st.pyplot(fig)

else:
    st.info("Agrega ítems para ver el resumen y la proyección.")
...
escenario = st.selectbox("Escenario de proyección", ["Peor de los casos", "Escenario normal", "Mejor de los casos"])
meses = st.slider("Meses de proyección", 0, 36, 6)
...
if st.session_state.items:
    ...
    st.write(f"📊 **Total proyectado a {meses} meses ({escenario}): {form_cop(total_proj)}**")
    ...
    ax2.set_title("Evolución del costo proyectado")
